...
Permitir que un usuario autenticado acceda a una página principal protegida y, desde ella, consulte los movimientos autorizados para su identidad, con estados claros durante la carga y ante la ausencia de datos o errores.

## ADDED Requirements

### Requirement: Acceso protegido a la página principal

El sistema SHALL ofrecer la ruta `/home` a usuarios que dispongan de un token de autenticación almacenado en la sesión del navegador.

Si un usuario sin token intenta acceder a `/home`, el sistema MUST redirigirlo a `/login`.

#### Scenario: Acceso autenticado a Home

* **WHEN** un usuario con un token de autenticación almacenado accede a `/home`
* **THEN** el sistema permite el acceso a la página principal

#### Scenario: Acceso no autenticado a Home

* **WHEN** un usuario sin token de autenticación accede a `/home`
* **THEN** el sistema lo redirige a `/login`

---

### Requirement: Navegación desde Home hacia movimientos

La página `/home` SHALL ofrecer al usuario una opción para acceder al listado de sus movimientos.

#### Scenario: Acceso al listado desde Home

* **WHEN** un usuario autenticado selecciona la opción de ver sus movimientos desde `/home`
...
Cuando un usuario complete correctamente el inicio de sesión y la API devuelva un token válido, el sistema MUST conservar dicho token mediante el mecanismo de autenticación existente y MUST navegar a `/home`.

#### Scenario: Redirección posterior al inicio de sesión

* **WHEN** un usuario inicia sesión correctamente y el token ha sido almacenado
